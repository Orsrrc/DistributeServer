...
/*
    HTTP服务器将客户机的下载功能包装在针对HTTP请求GET方法的处理中，
直接将从存储服务器下载得到的文件内容数据以HTTP响应的形式回传给
HTTP客户机
因此 该HTTP服务器不对请求POST做出回应
*/
    ...
    acl::HttpServletResponse& res)
{
    ...
    if (!path.ncompare(ROUTE_FILES, strlen(ROUTE_FILES)))  //字符串匹配 判断路径字符串中是否有ROUTE_FILES 这个变量的参数
    {
        // 处理文件路由
        files(req, res);
    }
...
{
    // 发送响应
    return res.write(NULL, 0);// 发送结束标志
}
